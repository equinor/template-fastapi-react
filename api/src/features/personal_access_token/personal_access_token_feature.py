...
def new_personal_access_token(
    ...
    return use_case_executor(create_pat_use_case, user, scope, time_to_live)
...
def revoke_personal_access_token(token_id: str, user: User = Depends(auth_with_jwt)):
    return use_case_executor(delete_pat_by_token_id_and_user_use_case, token_id, user)
...
def list_all_pats(user: User = Depends(auth_with_jwt)) -> list[PATData]:
    return use_case_executor(get_pat_by_user_use_case, user)
