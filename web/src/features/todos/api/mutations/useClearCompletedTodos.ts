...
import { deleteTodoById } from '@/api-generated'
...
export const useClearCompletedTodos = () => {
  ...
  return useMutation({
    mutationFn: async (): Promise<ClearResult> => {
      ...
      const results = await Promise.allSettled(ids.map((id) => deleteTodoById({ path: { id }, throwOnError: true })))
    ...
    onSuccess: ({ deleted, failed }) => {
      ...
      // global message via `meta.skipNotification`.
