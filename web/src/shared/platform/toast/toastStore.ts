// Tiny pub/sub store for toast notifications. Framework-free so it can
// be called from anywhere — including non-React code like the global
// `MutationCache` callbacks in `app/bootstrap/createQueryClient`.
//
// The store holds the queue of currently-visible toasts;
// `<ToastContainer />` subscribes via `useSyncExternalStore`. Toasts
// auto-dismiss after `autoHideMs`.
//
...
//     The existing toast keeps its place in the queue.
