...
# Currently oauth2-proxy does not support reading the redis password from a file natively
...
main() {
    if ! echo "$*" | grep -qE -- '--cookie-secret( +|=)?'; then
        if [ -n "${OAUTH2_PROXY_COOKIE_SECRET:-}" ]; then
            echo "Using provided OAUTH2_PROXY_COOKIE_SECRET for cookie secret"
            set -- "$@" --cookie-secret "$OAUTH2_PROXY_COOKIE_SECRET"
        elif [ -n "${OAUTH2_PROXY_COOKIE_SECRET_FILE:-}" ]; then
            echo "Using provided OAUTH2_PROXY_COOKIE_SECRET_FILE for cookie secret"
